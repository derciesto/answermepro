            ...
            if ($contact->is_contact == "No") {
                if ($contact->user) {
                    ...
                    if ($contact->user->profile_picture != "") {
                        ...
                        $thumbImage = asset("uploads/profile_pictures/thumb_" . $contact->user->profile_picture);
                    }
                    $userData = ['id' => $contact->user->id, "name" => $contact->user->name, "profile_picture" => $profile_picture, 'thumb' => $thumbImage];
                } else {
                    ...
                    if ($toUser) {
                        ...
                        if ($toUser->profile_picture != "") {
                            ...
                            $thumbImage = asset("uploads/profile_pictures/thumb_" . $toUser->profile_picture);
                        }
                        $userData = ['id' => $toUser->id, "name" => $toUser->name, "profile_picture" => asset("uploads/profile_pictures/" . $toUser->profile_picture), 'thumb' => $thumbImage];
                    }
                }
            }

            $contactData[] = ["id" => $contact->id, "name" => $contact->name, "contact_number" => $contact->contact_number, "profile_picture" => $profile_picture, "user" => (object)$userData, "is_user" => $isUser, 'thumb' => $thumbImage];
        }
